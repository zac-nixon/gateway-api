	...
	Test: func(t *testing.T, suite *confsuite.ConformanceTestSuite) {
		...
		t.Run("UDP traffic should be distributed across the weighted backends", func(t *testing.T) {
			...
			for i := range weight.MaxTestRetries {
				err := assertUDPWeightedDistribution(gwAddr, expectedWeights, 0.03)
				if err == nil {
					return
				}
...
func assertUDPWeightedDistribution(gwAddr string, expectedWeights map[string]float64, tolerance float64) error {
	const (
	...
	for range totalRequests {
		g.Go(func() error {
			pod, err := udpEchoSendOnce(gwAddr, probeTimeout)
			if err != nil {
				return err
			}
...
func udpEchoSendOnce(gwAddr string, timeout time.Duration) (string, error) {
