...
* Define a standard Gateway API Route resource for UDP traffic.
...
* Define rich UDP-specific matching semantics such as address matching, port matching, or payload inspection.
...
Gateway API currently supports TCP based routing, but lacks a standard resource for UDP traffic routing. 
UDP is a fundamental transport protocol used by many high performance applications and services:
...
UDPRoute allows for a consistent networking configuration management and enables multi-tenant load balancing experiences.
...
A common use-case is to expose the same service over UDP and TCP. An Implementation MAY listener for both UDP and TCP traffic
